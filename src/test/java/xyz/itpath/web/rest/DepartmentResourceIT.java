...
import xyz.itpath.domain.Department;
import xyz.itpath.domain.DepartmentAuthority;
import xyz.itpath.domain.User;
import xyz.itpath.repository.DepartmentRepository;
import xyz.itpath.service.DepartmentQueryService;
import xyz.itpath.service.DepartmentService;
import xyz.itpath.service.criteria.DepartmentCriteria;
...
class DepartmentResourceIT {
    ...
    void getAllDepartmentsWithEagerRelationshipsIsEnabled() throws Exception {
        when(departmentServiceMock.findAllWithEagerRelationships(any())).thenReturn(new Page().setRecords(new ArrayList<>()));
    ...
    @Test
    @Transactional
    void getAllDepartmentsByDepartmentAuthoritiesIsEqualToSomething() throws Exception {
        // Initialize the database
        departmentRepository.insert(department);
        DepartmentAuthority departmentAuthorities = DepartmentAuthorityResourceIT.createEntity();
        department.addDepartmentAuthorities(departmentAuthorities);
        departmentRepository.insert(department);
        Long departmentAuthoritiesId = departmentAuthorities.getId();

        // Get all the departmentList where departmentAuthorities equals to departmentAuthoritiesId
        defaultDepartmentShouldBeFound("departmentAuthoritiesId.equals=" + departmentAuthoritiesId);

        // Get all the departmentList where departmentAuthorities equals to (departmentAuthoritiesId + 1)
        defaultDepartmentShouldNotBeFound("departmentAuthoritiesId.equals=" + (departmentAuthoritiesId + 1));
    }
    ...
    void partialUpdateDepartmentWithPatch() throws Exception {
        ...
        partialUpdatedDepartment.code(UPDATED_CODE).phoneNum(UPDATED_PHONE_NUM).contact(UPDATED_CONTACT).createTime(UPDATED_CREATE_TIME);
