package xyz.itpath.system.web.rest;

import static org.assertj.core.api.Assertions.assertThat;
import static org.hamcrest.Matchers.hasItem;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;
import static xyz.itpath.web.rest.TestUtil.createFormattingConversionService;
import static xyz.itpath.web.rest.TestUtil.sameInstant;
import static xyz.itpath.web.rest.TestUtil.sameInstant;
...
import xyz.itpath.IntegrationTest;
import xyz.itpath.LwjsApp;
import xyz.itpath.domain.enumeration.MessageSendType;
import xyz.itpath.domain.enumeration.SendStatus;
import xyz.itpath.system.domain.SmsMessage;
import xyz.itpath.system.repository.SmsMessageRepository;
import xyz.itpath.system.service.SmsMessageQueryService;
...
import xyz.itpath.web.rest.errors.ExceptionTranslator;
...
class SmsMessageResourceIT {
    ...
    @Test
    @Transactional
    void getAllSmsMessagesByTitleIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where title not equals to DEFAULT_TITLE
        defaultSmsMessageShouldNotBeFound("title.notEquals=" + DEFAULT_TITLE);

        // Get all the smsMessageList where title not equals to UPDATED_TITLE
        defaultSmsMessageShouldBeFound("title.notEquals=" + UPDATED_TITLE);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesBySendTypeIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where sendType not equals to DEFAULT_SEND_TYPE
        defaultSmsMessageShouldNotBeFound("sendType.notEquals=" + DEFAULT_SEND_TYPE);

        // Get all the smsMessageList where sendType not equals to UPDATED_SEND_TYPE
        defaultSmsMessageShouldBeFound("sendType.notEquals=" + UPDATED_SEND_TYPE);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesByReceiverIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where receiver not equals to DEFAULT_RECEIVER
        defaultSmsMessageShouldNotBeFound("receiver.notEquals=" + DEFAULT_RECEIVER);

        // Get all the smsMessageList where receiver not equals to UPDATED_RECEIVER
        defaultSmsMessageShouldBeFound("receiver.notEquals=" + UPDATED_RECEIVER);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesByParamsIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where params not equals to DEFAULT_PARAMS
        defaultSmsMessageShouldNotBeFound("params.notEquals=" + DEFAULT_PARAMS);

        // Get all the smsMessageList where params not equals to UPDATED_PARAMS
        defaultSmsMessageShouldBeFound("params.notEquals=" + UPDATED_PARAMS);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesBySendTimeIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where sendTime not equals to DEFAULT_SEND_TIME
        defaultSmsMessageShouldNotBeFound("sendTime.notEquals=" + DEFAULT_SEND_TIME);

        // Get all the smsMessageList where sendTime not equals to UPDATED_SEND_TIME
        defaultSmsMessageShouldBeFound("sendTime.notEquals=" + UPDATED_SEND_TIME);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesBySendStatusIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where sendStatus not equals to DEFAULT_SEND_STATUS
        defaultSmsMessageShouldNotBeFound("sendStatus.notEquals=" + DEFAULT_SEND_STATUS);

        // Get all the smsMessageList where sendStatus not equals to UPDATED_SEND_STATUS
        defaultSmsMessageShouldBeFound("sendStatus.notEquals=" + UPDATED_SEND_STATUS);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesByRetryNumIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where retryNum not equals to DEFAULT_RETRY_NUM
        defaultSmsMessageShouldNotBeFound("retryNum.notEquals=" + DEFAULT_RETRY_NUM);

        // Get all the smsMessageList where retryNum not equals to UPDATED_RETRY_NUM
        defaultSmsMessageShouldBeFound("retryNum.notEquals=" + UPDATED_RETRY_NUM);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesByFailResultIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where failResult not equals to DEFAULT_FAIL_RESULT
        defaultSmsMessageShouldNotBeFound("failResult.notEquals=" + DEFAULT_FAIL_RESULT);

        // Get all the smsMessageList where failResult not equals to UPDATED_FAIL_RESULT
        defaultSmsMessageShouldBeFound("failResult.notEquals=" + UPDATED_FAIL_RESULT);
    }
    ...
    @Test
    @Transactional
    void getAllSmsMessagesByRemarkIsNotEqualToSomething() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);

        // Get all the smsMessageList where remark not equals to DEFAULT_REMARK
        defaultSmsMessageShouldNotBeFound("remark.notEquals=" + DEFAULT_REMARK);

        // Get all the smsMessageList where remark not equals to UPDATED_REMARK
        defaultSmsMessageShouldBeFound("remark.notEquals=" + UPDATED_REMARK);
    }
    ...
    @Test
    @Transactional
    void putNewSmsMessage() throws Exception {
        // Initialize the database
        smsMessageRepository.insert(smsMessage);
        ...
        SmsMessage updatedSmsMessage = smsMessageRepository.findById(smsMessage.getId()).get();
        // Disconnect from session so that the updates on updatedSmsMessage are not directly saved in db
        updatedSmsMessage
            .title(UPDATED_TITLE)
            .sendType(UPDATED_SEND_TYPE)
            .receiver(UPDATED_RECEIVER)
    ...
    void putNonExistingSmsMessage() throws Exception {
        int databaseSizeBeforeUpdate = smsMessageRepository.findAll().size();
        smsMessage.setId(count.incrementAndGet());
    ...
    void partialUpdateSmsMessageWithPatch() throws Exception {
        ...
        partialUpdatedSmsMessage.title(UPDATED_TITLE).content(UPDATED_CONTENT).retryNum(UPDATED_RETRY_NUM).remark(UPDATED_REMARK);
