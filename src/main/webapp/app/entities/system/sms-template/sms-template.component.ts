...
@Component
export default class SmsTemplateComponent extends Vue {
  ...
  public smsTemplates: ISmsTemplate[] = [];
  ...
  public clickSmsTemplateId = null;
  public searchValue = '';
  ...
  public prepareRemove(instance: ISmsTemplate): void {
  ...
  public removeById(removeId: number): void {
    this.smsTemplateService()
      ...
      .then((res: AxiosResponse) => {
        const message = this.$t('lwjsApp.smsTemplate.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: ISmsTemplate): void {
    if (this.showInOther || this.editInModal) {
      this.smsTemplateId = row.id;
