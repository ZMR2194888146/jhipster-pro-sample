...
export default class ApiPermissionComponent extends Vue {
  ...
  public clickApiPermissionId = null;
  public searchValue = '';
  ...
  public removeById(removeId: number): void {
    this.apiPermissionService()
      ...
      .then((res: AxiosResponse) => {
        const message = this.$t('lwjsApp.systemApiPermission.deleted', { param: this.removeId }).toString();
  ...
  public addChild(parent: IApiPermission): void {
    if (this.showInOther || this.editInModal) {
      this.updateModalVisible = true;
      this.parentId = parent.id;
    } else {
      // @ts-ignore
      this.$router.push({ name: 'system-api-permission-new', params: { parentId: parent.id }, append: true });
    }
  }
