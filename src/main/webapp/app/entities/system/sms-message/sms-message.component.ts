...
@Component
export default class SmsMessageComponent extends Vue {
  ...
  public smsMessages: ISmsMessage[] = [];
  ...
  public prepareRemove(instance: ISmsMessage): void {
  ...
  public removeById(removeId: number): void {
    this.smsMessageService()
      ...
      .then((res: AxiosResponse) => {
        const message = this.$t('lwjsApp.smsMessage.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: ISmsMessage): void {
    if (this.showInOther || this.editInModal) {
      this.smsMessageId = row.id;
