...
@Component
export default class SysLogComponent extends Vue {
  ...
  public totalItems = 0;
  public omitFields = ['id', 'requestParam'];
  public sysLogs: ISysLog[] = [];
  ...
  public prepareRemove(instance: ISysLog): void {
  ...
  public removeById(removeId: number): void {
    this.sysLogService()
      ...
      .then((res: AxiosResponse) => {
        const message = this.$t('lwjsApp.sysLog.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: ISysLog): void {
    if (this.showInOther || this.editInModal) {
      this.sysLogId = row.id;
