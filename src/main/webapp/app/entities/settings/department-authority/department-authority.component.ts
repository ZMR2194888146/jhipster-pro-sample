...
@Component
export default class DepartmentAuthorityComponent extends Vue {
  ...
  public mapOfFilter: { [key: string]: any } = {
    ...
    apiPermissions: { list: [], value: [], type: 'one-to-many' },
    viewPermissions: { list: [], value: [], type: 'one-to-many' },
  ...
  public clickDepartmentAuthorityId = null;
  public searchValue = '';
  ...
  public loadAll(): void {
    ...
    const paginationQuery = {
      listModelName: 'DepartmentAuthority',
  ...
  public prepareRemove(instance: IDepartmentAuthority): void {
  ...
  public removeById(removeId: number): void {
    this.departmentAuthorityService()
      ...
      .then((res: AxiosResponse) => {
        const message = this.$t('lwjsApp.settingsDepartmentAuthority.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: IDepartmentAuthority): void {
    if (this.showInOther || this.editInModal) {
      this.departmentAuthorityId = row.id;
  ...
  public updateModalCancel(e: any) {
    this.departmentAuthorityId = null;
  ...
  public initRelationships(): void {
    this.loading = true;
    Promise.all([this.departmentService().tree()])
      .then(([departments]) => {
