package xyz.itpath.web.rest.standalone;
...
import xyz.itpath.security.UserNotActivatedException;
import xyz.itpath.security.jwt.JWTFilter;
import xyz.itpath.security.jwt.TokenProvider;
...
import xyz.itpath.sms.model.SmsCode;
import xyz.itpath.sms.model.SmsData;
import xyz.itpath.web.rest.vm.LoginMobileVM;
...
public class UserSmsController {
    ...
    public ResponseEntity<JWTToken> authorizeByMobile(@Valid @RequestBody LoginMobileVM loginMobileVM) {
        ...
        // Result<Void> result = daYuSmsService.verify(loginMobileVM.getMobile(), loginMobileVM.getCode(), loginMobileVM.getImageCode(), "lwjs");
        if (result) {
            // 通过mobile查找用户信息。
    ...
    public ResponseEntity<Result<String>> getSmsCodeByCurrentUser(String mobile, @RequestParam(name = "imageCode", required = false) String imageCode) {
        ...
        if (SecurityUtils.getCurrentUserLogin().isPresent()) {
            ...
            if (!existsMobile) {
                // 不存在，发送验证码
                Result<String> result = daYuSmsService.sendVerifyCode(mobile, "lwjs");
