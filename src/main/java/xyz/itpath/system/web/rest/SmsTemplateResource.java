...
import xyz.itpath.system.repository.SmsTemplateRepository;
import xyz.itpath.system.service.SmsTemplateQueryService;
import xyz.itpath.system.service.SmsTemplateService;
import xyz.itpath.system.service.criteria.SmsTemplateCriteria;
 ...
 * 管理实体{@link xyz.itpath.system.domain.SmsTemplate}的REST Controller。
 */
@RestController
@RequestMapping("/api")
...
public class SmsTemplateResource {
    ...
    public ResponseEntity<SmsTemplateDTO> partialUpdateSmsTemplate(
        ...
        if (smsTemplateRepository.findById(id).isPresent()) {
            throw new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound");
        }
    ...
    public ResponseEntity<SmsTemplateDTO> updateSmsTemplateDTOBySpecifiedFields(
        @RequestBody SmsTemplateDTOAndSpecifiedFields smsTemplateDTOAndSpecifiedFields
    ) {
        log.debug("REST request to update SmsTemplateDTO : {}", smsTemplateDTOAndSpecifiedFields);
    ...
    /**
     * {@code PUT  /sms-templates/specified-field} : Updates an existing smsTemplateDTO by specified field.
     *
     * @param smsTemplateDTOAndSpecifiedFields the smsTemplateDTO and specifiedFields to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated smsTemplateDTO,
     * or with status {@code 400 (Bad Request)} if the smsTemplateDTO is not valid,
