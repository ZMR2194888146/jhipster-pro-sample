...
import xyz.itpath.domain.DataDictionary;
import xyz.itpath.repository.DataDictionaryRepository;
import xyz.itpath.service.dto.DataDictionaryDTO;
import xyz.itpath.service.mapper.DataDictionaryMapper;
...
public class DataDictionaryService extends BaseServiceImpl<DataDictionaryRepository, DataDictionary> {
    ...
    private final List<String> relationCacheNames = Arrays.asList(
        xyz.itpath.domain.DataDictionary.class.getName() + ".parent",
        xyz.itpath.domain.DataDictionary.class.getName() + ".children"
    );
    ...
    public Optional<DataDictionaryDTO> partialUpdate(DataDictionaryDTO dataDictionaryDTO) {
        ...
        return dataDictionaryRepository
            ...
            .map(
                tempDataDictionary -> {
                    dataDictionaryRepository.save(tempDataDictionary);
                    return dataDictionaryMapper.toDto(dataDictionaryRepository.selectById(tempDataDictionary.getId()));
                }
            );
    }
    ...
    public IPage<DataDictionaryDTO> findAllTop(Page<DataDictionary> pageable) {
        ...
        return dataDictionaryRepository
            ...
            .convert(
                dataDictionary -> {
                    Binder.bindRelations(dataDictionary, new String[] { "parent" });
                    return dataDictionaryMapper.toDto(dataDictionary);
                }
            );
    }
        ...
        fieldNames.forEach(
            fieldName -> {
                updateWrapper.set(
                    ...
                    BeanUtil.getFieldValue(changeDataDictionaryDTO, fieldName)
                );
            }
        );
    ...
    public DataDictionaryDTO updateBySpecifiedField(DataDictionaryDTO changeDataDictionaryDTO, String fieldName) {
        DataDictionaryDTO update = new DataDictionaryDTO();
        BeanUtil.setFieldValue(update, "id", changeDataDictionaryDTO.getId());
        BeanUtil.setFieldValue(update, fieldName, BeanUtil.getFieldValue(changeDataDictionaryDTO, fieldName));
        this.updateEntity(dataDictionaryMapper.toEntity(update));
        return findOne(changeDataDictionaryDTO.getId()).get();
    }

    // 清除children缓存
    private void clearChildrenCache() {
        Objects.requireNonNull(cacheManager.getCache(xyz.itpath.domain.DataDictionary.class.getName() + ".children")).clear();
    }
